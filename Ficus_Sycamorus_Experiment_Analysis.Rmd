...
syc_stops_north<-read_csv("data/bats_visits_north_ficus_30_m_Radius.csv") # Tree visits within 30 m buffer around the Northern target F.sycomorus tree 
...
manip_syc_tags<-read_csv("data/Tags_manipulated_and_visiting.csv")
...
man_tags_nights<-manip_syc_tags %>% dplyr::group_by(Nights_after_manipulation) %>% summarise("sum_bats_night"=sum(length(unique(tags))))
fifth_night<-data.frame("Nights_after_manipulation"=5,"sum_bats_night"=0)
...
for (i in 1:6) {
    proportions_file <- sprintf("proportions_no_replacement/proportion%d_Nights_at_least_70.rds", i)
    ...
    summary_file <- sprintf("proportions_no_replacement/interval_summary_%d_Nights.rds", i)
...
for (i in 1:6) {
    file_name <- sprintf("proportions_no_replacement/proportion%d_Nights_at_least_70.rds", i)
