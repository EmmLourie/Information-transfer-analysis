...
**Get data of tree-visits (routine movement) to the focal trees** 
```{r}
all_stops<-read_csv("data/all_tree_visits_main.csv") # Table of all tree visits ("stops") 
...
**During the manipulation, most bats (11/14) arrived 1-3 nights after the manipulation to the target tree, while others up to 6 nights after.**
**Hence, I asked whether there was a visit within 3 nights interval during non-manipulation periods, to estimate the probability of visiting the target tree during routine movements and compare it to the manipulation results after**
```{r}
...
before_man<-seq(man_start-1,start_date, (-1)*test_interval)
after_man<-seq(man_end+1, end_date,test_interval)
man<-seq(man_start,man_end,test_interval)
...
**Summarize - counting the number of visits (unique to each tag, not accounting for multiple visits of the same bat during the three-nights interval) to *F. sycomorus* and the overall number of tagged bats during the same three-nights intervals**
(Removing the manipulation period)  
```{r}
syc_dt_sum_t<-syc_dt_all %>% group_by(fruits,TAG, tdate) %>% summarise("n_syc"=length(syc[syc=="yes"]))
syc_dt_sum_t$syc_vis<-ifelse(syc_dt_sum_t$n_syc>0,1,0) # counting each bats visit during the 3-night interval as either visited (=1) or not. 
table(syc_dt_sum_t$fruits) 

# This summarizes: the number of three-night intervals where any bat visited the focal trees (n_vis_syc), out of the total number of bats tracked during the same 3-nights intervals. 
...
syc_sum_test<-syc_sum_triple %>% group_by(fruits) %>% summarise("n_vis" = sum(n_syc_vis),"n_tags"=sum(n_tags)) # "n_vis" = non-unique number of tags visiting the focal tree in 3-nights intervals. "n_tags" the total number of bats tracked during the same periods (here too, not the unique number of tags)
