...
# Get stops data of bats tracked at least three nights
```{r pressure, echo=FALSE}
dt_stops<-read_csv("Data/data_information_trees.csv")
...
# WeekGlobal<-lubridate::week(dt_stops$DateTimeStart)
# YearGlobal<-lubridate::year(dt_stops$DateTimeStart)
# dt_stops$WeekGlobal<-paste0(WeekGlobal,YearGlobal)
# dt_stops<-dt_stops %>% group_by(WeekGlobal) %>% mutate("DateWeek"=min(date_global))
```
...
# Get the individul bats' measurments (taken upon capture)
```{r}
...
# Find pairs of bats that were tracked together simoultaniouslly, with help from the "data.table::foverlaps" function. 
```{r}
...
# Find psuedo following events - summarize outcomes of tree-encounters in a table per night and save it externally. 
Tables shall be saved in a folder named "tree_encounters".

This is the heaviest part of the code, and should take ~ 15 mins to run. If you wish to skip it, the results are already uploaded to the source-code folder "data\tree_encounters"
```{r}
